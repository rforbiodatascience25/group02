---
title: "Group Assignment 7 - Group 2"
format:
  html:
...
We use augment() to add PCA coordinates to the original data and color samples by prognosis. The plot shows how patients cluster along the first two principal components based on their gene expression.

```{r}
gravier_PCA |> 
  augment(gravier_clean_aug) |>
  mutate(prognosis=recode(as.factor(prognosis),
         "0" = "good",
         "1" = "poor")) |>
ggplot(aes(.fittedPC1, .fittedPC2, color = prognosis)) + 
  geom_point(size = 1.5, alpha = 0.8) +
  scale_color_manual(
    values = c("good" = "#2297E6", 
               "poor" = "#FF69B4") 
  ) +
  labs(
    title = "PCA of Breast Cancer Gene Expression",
    x = "PC1",
    y = "PC2",
    color = "Prognosis"
  ) +
...
gravier_PCA |> 
  ...
  labs(title = "Loadings plot",
       subtitle = "15 random genes") +
  theme_minimal_grid()
```

On the loadings plot it can be seen that of the 15 random genes g11D09 is the only one that contributes positively to PC2. The genes that contribute most positively to PC1 are g7C02, g1int627 and g1int421. The genes that contribute most negatively to PC1 are g1CNS215, g1int108, and g1int1404. Unfortunately this can not be used to say something about their effect on prognosis because PC1 and PC2 don't separate good and poor prognosis as seen on the score plot above
